...
5-1. 위험등급(상/중/하)은 절대 감으로 판단하지 마세요. search_accident_cases 결과에 포함된
   "통계기반등급"(실제 원본 데이터의 사망자/부상자 수로 계산된 값)을 그대로 사용하세요.
   - 근거로 인정한 사례가 여러 건이면, 그중 가장 심각한(상 > 중 > 하) 등급을 최종 위험등급으로
     쓰세요.
   - 보고서의 "위험등급" 항목에는 반드시 근거로 삼은 통계기반등급과 그 사례의 사망자/부상자
     수를 함께 명시하세요 (예: "상 (근거 사례 사망자 1명)").
...
### [공정명]
- **위험요소**: (구체적 위험 상황)
- **위험등급**: 상/중/하 (근거 사례의 통계기반등급 그대로 사용 + 사망자/부상자 수 명시)
